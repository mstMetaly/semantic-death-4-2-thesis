...
from imageio.v2 import imread#updated 
from multiprocessing import Pool, cpu_count
from multiprocessing.pool import ThreadPool
# from scipy.ndimage.interpolation import zoom
from scipy.ndimage import zoom#updated
...
class SegmentationData(AbstractSegmentation):
    ...
    def __init__(self, directory, categories=None, require_all=False):
        ...
        primary_cats = self.primary_categories_per_index()
        print(f"\nDEBUG: primary_categories_per_index() result:")
        print(f"  Shape: {primary_cats.shape}")
        print(f"  Unique values: {np.unique(primary_cats)}")
        print(f"  Value counts: {np.bincount(primary_cats.astype(int))}")
        print(f"  First 20 values: {primary_cats[:20]}")
        print(f"  All values same? {np.all(primary_cats == primary_cats[0])}")
        self.labelcat = self.onehot(primary_cats)
        print(f"DEBUG: labelcat after onehot:")
        print(f"  Shape: {self.labelcat.shape}")
        print(f"  Unique rows: {len(np.unique(self.labelcat.reshape(-1, self.labelcat.shape[-1]), axis=0))}")
        print(f"  First 5 rows:\n{self.labelcat[:5, :]}")
        print(f"  All rows identical? {np.all(self.labelcat == self.labelcat[0], axis=1).all()}")
...
class SegmentationPrefetcher:
    ...
    def __init__(self, segmentation, split=None, randomize=False,
        ...
        #  n_procs = cpu_count()
        n_procs = min(cpu_count(), 8)#updated
        if thread:
        ...
        # self.indexes = range(start, end)#updated
        self.indexes = list(range(start, end))#updated
        if split:
    ...
    def fetch_tensor_batch(self, bgr_mean=None, global_labels=False):
        """Returns a single batch of tensors, or None at end."""
        batch = self.fetch_batch()
        if batch is None:
            return None
    ...
    def tensor_batches(self, bgr_mean=None, global_labels=False):
        """Iterator for all tensor batches (never yields None)."""
        while True:
