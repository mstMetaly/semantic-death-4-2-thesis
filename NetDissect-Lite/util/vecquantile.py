...
class QuantileVector:
    ...
    def __init__(self, depth=1, resolution=24 * 1024, buffersize=None,
        ...
        self.extremes.fill(numpy.nan)#updated
        self.size = 0
    ...
    def add(self, incoming):
        ...
        chunksize = int(numpy.ceil(self.buffersize / self.samplerate))#updated
        for index in range(0, len(incoming), chunksize):#updated
            ...
            if available == 0:
                if not self._shift():
                    ...
                    if self.samplerate >= 0.5:
                        print('SAMPLING')
                    self._scan_extremes(incoming)
                    incoming = incoming[self.random.binomial(1, 0.5, len(incoming))]
                    index = 0
                    supplied = len(incoming)
    ...
    def _next_capacity(self):
        cap = float(numpy.ceil(self.resolution * numpy.power(0.67, len(self.data))))
        if cap < 2:
            return 0
